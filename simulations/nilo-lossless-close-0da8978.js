// Nilo (AI agent built with Claude): fork test of the lossless close at f015382/0da8978 (bounty muerdzoc805a745ecc99).
// In this design sync itself rolls a sold-out epoch (index under MINT_FLOOR). Same fills that froze the rung at
// 24f3e23; two automatic rolls with the first epoch's members still in; everyone paid exactly from the reserve.
...
const keeper = mk(871);
const CONTRACTS = process.env.CONTRACTS || '../../jing-v3-0da8978/contracts/';
...
async function scenario(side,kind) {
 ...
 // 2. Same real fills as the freeze repro; this time sync alone must close the epoch (no deposit needed).
 check('reserve empty at start',await reserved(),0n);
 await sellTo(residualTarget);await sync();
 const r1=await show('after fills + sync');
 check('sync rolled epoch 0 by itself (no deposit)',r1.epoch,1n);
 check('index restarts at SCALE',r1.index,SCALE);
 check('no shares left in the new epoch',r1.shares,0n);
 const aOwed=await posOf(alice),bOwed=await posOf(bob);
 console.log(`  owed after roll 1: alice back=${aOwed.back} proceeds=${aOwed.proceeds}; bob back=${bOwed.back} proceeds=${bOwed.proceeds}`);
 const res1=await reserved();console.log(`  reserve after roll 1: ${res1}`);
 check('reserve covers epoch 0 unsold shares',res1,v=>v>=aOwed.back+bOwed.back);
 // 3. Carol joins the empty pool (takes any orphan units in), her epoch is filled into its tail, and sync rolls again.
 await tx('carol deposits into the fresh epoch',carol,rung,'deposit',[uintCV(big)],ok);
 await sellTo(residualTarget);await sync();
 const r2=await show('after second fills + sync');
 check('sync rolled epoch 1 by itself',r2.epoch,2n);
 const cOwed=await posOf(carol);
 console.log(`  owed after roll 2: carol back=${cOwed.back} proceeds=${cOwed.proceeds}`);
 const res2=await reserved();console.log(`  reserve after roll 2: ${res2}`);
 check('reserve covers both closed epochs at once',res2,v=>v>=aOwed.back+bOwed.back+cOwed.back);
 // 4. All three old members exit after both rolls: exact proceeds + unsold share each.
 for(const [nm,who,o] of [['bob',bob,bOwed],['alice',alice,aOwed],['carol',carol,cOwed]]){
  const inS=await wallet(side,who),inO=await wallet(opposite(side),who);
  await tx(`${nm} (closed epoch) exits`,who,rung,'withdraw',[uintCV(big*100n),someCV(update(signed))],ok);
 ...
 const left=await reserved();console.log(`  reserve left after all three: ${left}`);
 check('reserve never short, only rounding dust left (< 3 members)',left,v=>v>=0n&&v<3n);
 // 5. A fresh depositor joins the empty epoch 2 and can leave with at least the deposit.
 const dave=mk(886);await fund(side,dave,big*4n);
 await tx('dave deposits into the empty epoch',dave,rung,'deposit',[uintCV(big)],ok);
 const dIn=await wallet(side,dave);
 await exit(dave);
 const got=await wallet(side,dave)-dIn;console.log(`  dave deposited ${big}, got back ${got}`);
 check('fresh depositor exits with at least the deposit (minus rounding)',got,v=>v>=big-2n);
